...
namespace Slickflow.Engine.Business.Manager
{
    internal class ProcessInstanceManager : ManagerBase
    {
        ...
        {
            ProcessInstanceEntity instance = null;
            var sql = @"SELECT 
                            TOP 1 PI.*
                        FROM WfProcessInstance PI
                        INNER JOIN WfActivityInstance AI
                            ON PI.ID=AI.ProcessInstanceID
                        WHERE AI.ID=@activityInstanceID";
            var list = Repository.Query<ProcessInstanceEntity>(sql, new { activityInstanceID = activityInstanceID }).ToList();
            ...
            {
                throw new ApplicationException(string.Format("异常数据,根据活动实例ID查不到流程实例，活动实例ID:{0}", 
                    activityInstanceID));
            }
            return instance;
        }
